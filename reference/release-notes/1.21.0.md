...
- In the [AMS HTTP API](/reference/api-reference/ams-api.md), `/1.0` endpoint exposes the cluster ID and name to enable identifying a subcluster.<!--AC-2148-->
...
- When using the [AMS HTTP API](/reference/api-reference/ams-api.md), the API response for `GET /1.0/instances` does not properly return information for the fields `config` and `resources`. <!--AC-2156-->
