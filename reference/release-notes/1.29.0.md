...
Anbox Cloud 1.29.0 is a minor release. To understand minor and patch releases, see {ref}`Release notes <ref-release-notes>`.
...
Use the {ref}`amc publish command <howto-publish-instance-as-image>` (via `amc publish <instance name> <image name>`) to create reusable images from running or stopped Anbox Cloud instances. This captures all file system modifications, enabling the creation of custom base images with pre-installed software and configurations.

### Runtime bitrate control for WebRTC

Users now have the ability to {ref}`dynamically adjust the minimum and maximum video bitrate <anbox-https-api>` for WebRTC streaming at runtime via the 1.0/platform HTTP API. This allows for application-specific optimization, leading to reduced network bandwidth usage and operational costs.

### Optimized disk quota

The AMS scheduler now uses a two-tier disk quota validation to prevent 'false exhaustion' and improve disk use, especially with ZFS storage drivers. This feature introduces a {ref}`new **--no-disk-reserved** flag <howto-create-instance>` for allowing disk overcommitting (increasing node density) and enforces a mandatory 5% physical disk safety guard to ensure node stability.
...
This release introduces {ref}`fine-grained authorization <howto-auth>` through integration with OpenFGA and a configured OIDC provider, enhancing access control for the Anbox Management Service (AMS) and Dashboard.
...
See {ref}`How to upgrade Anbox Cloud <howto-upgrade-anbox-cloud>` for instructions on how to update your Anbox Cloud deployment to the 1.29.0 release.
